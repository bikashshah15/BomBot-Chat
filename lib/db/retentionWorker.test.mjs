...
test('failed extraction records a failure without postponing destruction; participant deletion preserves results', async context => {
  ...
  try {
    ...
    try {
      ...
      // Capture all evidence before the first assertion: cleanup removes this schema.
      const keysPresent=await Promise.all(ids.map(id=>sessionKeyVault.has(id)));
      ...
      const evidence=ids.map((id,index)=>({session_id:id,deadline_ms:deadlines.get(id),
        destruction_ms:destructionTimes.get(id)??null,key_present:keysPresent[index],
        key_met_deadline:destructionTimes.has(id)&&destructionTimes.get(id)<=Math.ceil(deadlines.get(id)),
      ...
      context.diagnostic(JSON.stringify({fixture_group:'concurrent_deadlines',sessions:evidence,
        retirement_rows:retirementRows.rows,logs,retry_attempts:retryAttempts,late_attempt_count:lateAttemptCount,
        late_key_count:lateKeyCount,
      ...
      assert.deepEqual(keysPresent,[false,false,false,false],
