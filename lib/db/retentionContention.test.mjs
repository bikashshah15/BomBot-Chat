...
// Structural early-opportunity and deadline tests; load runs are regression only.
test('early opportunities reach storage on the last pass, refresh activity, and preserve eight deadlines', async context => {
  ...
  try {
    ...
    const { startRetentionWorker, deleteSessionParticipantData, STORAGE_CUTOFF, RETIREMENT_LEAD, IDLE_WINDOW_MS, EXTRACTION_ATTEMPTS, FIRST_EXTRACTION_LEAD } = await import('./retentionWorker.ts');
    ...
    try {
      stop = await startRetentionWorker(async (id, revision) => {
    ...
    stop = await startRetentionWorker((id, revision) => {
    ...
    const keyPresent = await Promise.all(ids.map(id => vault.has(id)));
    const sessions = ids.map((id, index) => ({ session_id: id, deadline_ms: deadlines.get(id),
      destruction_ms: destruction.get(id) ?? null, key_present: keyPresent[index],
    ...
    assert.deepEqual(keyPresent, Array(ids.length).fill(false));
    assert.ok(Math.max(...clusteredIds.map(id => deadlines.get(id))) - Math.min(...clusteredIds.map(id => deadlines.get(id))) < 1000, 'all eight deadlines fall within one second');
    for (const value of lateness) assert.ok(value >= -RETIREMENT_LEAD && value <= 0, 'destruction remains within the five-second lead');
...
test('three extraction opportunities derive from both configured idle windows', async () => {
  const { extractionOpportunityOffsets, EXTRACTION_ATTEMPTS } = await import('./retentionWorker.ts');
