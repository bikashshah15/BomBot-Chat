 ...
 */
export async function readRetentionCounts(client: Pick<pg.Pool, 'query'>,
  measurementEnabled = Boolean(config.MEASURE_PROVIDER_MODULE)): Promise<RetentionCounts> {
  const measureCounts = measurementEnabled ? `
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  try {
    ...
    process.exitCode = counts.overdue_deletion_count > 0
      || (Boolean(config.MEASURE_PROVIDER_MODULE) && (counts.missing_measure_count ?? 0) > 0) ? 1 : 0;
  } catch {
