...
test('app-owned resolve preserves each terminal result by identity', async () => {
  const results = [
    {
      content: '',
      toolCalls: [],
  ...
  const client = {
    responses: {
      async create() {
        throw new Error('Resolve must not create a response');
      },
    },
  };
  const gateway = createLlmGateway({
  ...
  // INC-06: remove — these assertions cover the temporary resolve surface.
  const failed = await gateway.resolve({ result: results[0] });
  // INC-06: remove — these assertions cover the temporary resolve surface.
  const cancelled = await gateway.resolve({ result: results[1] });
  // INC-06: remove — these assertions cover the temporary resolve surface.
  const incomplete = await gateway.resolve({ result: results[2] });

  assert.strictEqual(failed, results[0]);
  assert.equal(failed.status, 'failed');
  assert.deepEqual(failed.error, {
    code: 'rate_limit_exceeded',
    message: 'Synthetic rate limit response failure',
  });
  assert.strictEqual(cancelled, results[1]);
  assert.equal(cancelled.status, 'cancelled');
  assert.equal(cancelled.error, undefined);
  assert.strictEqual(incomplete, results[2]);
...
test('all four LLM routes use the gateway without legacy model transport', async () => {
  ...
  for (const routePath of routePaths) {
    ...
    if (routePath.endsWith('run-status.ts')) {
      ...
      assert.match(source, /completeConversationMessages\(/);
    } else {
      assert.match(source, /completeConversationMessages\(/);
    }
    assert.doesNotMatch(
      source,
      /createBackgroundResponse|retrieveResponse|continueFunctionCallingLoop/,
    );
  }
});
