...
async function captureLocalSdkRequest(settingsOverrides = {}) {
  let recordedRequest;
...
test('local SDK transport ignores LLM_API_KEY (no Postgres)', async () => {
  const canary = 'CANARY-HOSTED-KEY-do-not-send-7f3c';
  const request = await captureLocalSdkRequest({ LLM_API_KEY: canary });

  assert.equal(request.headers.authorization, 'Bearer local-openai-compatible');
  assert.equal(JSON.stringify(request.headers).includes(canary), false);
  assert.equal(request.body.includes(canary), false);
});

test('local SDK transport uses its placeholder without a key (no Postgres)', async () => {
  const request = await captureLocalSdkRequest({ LLM_API_KEY: undefined });

  assert.equal(request.headers.authorization, 'Bearer local-openai-compatible');
});

test('local SDK request body is invariant when LLM_API_KEY is set (no Postgres)', async () => {
  const withKey = await captureLocalSdkRequest({
    LLM_API_KEY: 'CANARY-HOSTED-KEY-do-not-send-7f3c',
  });
  const withoutKey = await captureLocalSdkRequest({ LLM_API_KEY: undefined });

  assert.deepEqual(withKey.body, withoutKey.body);
});

test('local SDK request body is invariant when reasoning effort is set (no Postgres)', async () => {
  const withReasoning = await captureLocalSdkRequest({ LLM_REASONING_EFFORT: 'medium' });
  const withoutReasoning = await captureLocalSdkRequest({ LLM_REASONING_EFFORT: undefined });

  assert.deepEqual(withReasoning.body, withoutReasoning.body);
});

test('hosted gateway still requires LLM_API_KEY without an injected client (no Postgres)', () => {
  assert.throws(
    () => createLlmGateway({
      settings: gatewaySettings({ PROFILE: 'hosted', LLM_API_KEY: undefined }),
    }),
    { message: 'LLM_API_KEY is required for the hosted LLM provider' },
  );
});
...
test('hosted OpenAI provider sends app-owned history without server state', async () => {
  ...
  for (const outboundRequest of requests) {
    assert.equal('reasoning' in outboundRequest, false);
...
test('hosted OpenAI provider keeps sampling parameters with none reasoning', async () => {
  const requests = [];
  const client = {
    responses: {
...
test('hosted OpenAI provider sends medium reasoning without sampling parameters', async () => {
  const requests = [];
  const client = {
    responses: {
      async create(request) {
  ...
  assert.equal('top_p' in requests[0], false);
});
