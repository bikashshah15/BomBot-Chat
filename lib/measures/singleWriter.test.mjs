...
test('session_measures has exactly one source writer: the lib/db insert/upsert', async () => {
  ...
  const writer = await readFile(path.join(root, permittedWriter), 'utf8');
  ...
  assert.ok(files.includes(permittedWriter));
