...
test('valid chat and upload timing records serialize with fixed key order (no Postgres)', () => {
  assert.equal(formatTimingRecord({
    ...
    persist_ms: null,
    total_ms: 9,
  }), '{"event":"timing_v1","kind":"chat_turn","provider":"alternate","tools_enabled":true,"outcome":"completed","history_load_ms":1,"rounds":[{"db_prep_ms":2,"model_first_chunk_ms":3,"model_stream_ms":4,"db_append_ms":5,"input_tokens":6,"output_tokens":7,"tool_calls_requested":1}],"tools":[{"round":1,"tool":"query_cve_details","ms":8,"ok":true}],"persist_ms":null,"total_ms":9}');
...
test('client timing serialization contains only structural literals, provider, and numbers', () => {
  const serialized = formatTimingRecord({
    kind: 'client_turn',
    provider: 'primary',
  ...
  assert.doesNotMatch(serialized, /[a-z]{20,}/);
  assert.equal(record.event, 'timing_v1');
  assert.equal(record.kind, 'client_turn');
  for (const [key, value] of Object.entries(record)) {
    if (key === 'event' || key === 'kind') continue;
    assert.equal(typeof value === 'number' || value === 'primary' || value === 'alternate', true);
  }
});
