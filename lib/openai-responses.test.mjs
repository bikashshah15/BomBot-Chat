...
test('hosted model instructions remain byte-identical after OSV provenance becomes conditional', () => {
  ...
  assert.equal(Buffer.byteLength(hostedInstructions, 'utf8'), 7694);
  assert.equal(
    createHash('sha256').update(hostedInstructions, 'utf8').digest('hex'),
    '1428a5bd276a1fecf7996bf553f44adf350cbdde4d8e2b58a114ba0feb53c28c',
  );
});
...
test('shipped model instructions omit the removed severity-tag directive', () => {
  for (const mode of ['api', 'offline']) {
    assert.doesNotMatch(
      buildBombotInstructions(mode),
