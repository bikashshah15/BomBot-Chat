...
const { createFunctionCallExecutor } = await import('./openai-responses.ts');
...
test('offline model tools distinguish absence and source failure from clean results', async () => {
  const internalDiagnostic = 'database connection failed at /private/internal/osv.db';
  ...
  assert.ok(loggedFailures.every(values => values.some(value => (
    value instanceof Error && value.message.includes(internalDiagnostic)
  ))));
