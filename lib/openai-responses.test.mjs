...
test('hosted model instructions remain byte-identical after OSV provenance becomes conditional', () => {
  ...
  assert.equal(Buffer.byteLength(hostedInstructions, 'utf8'), 7817);
  assert.equal(
    createHash('sha256').update(hostedInstructions, 'utf8').digest('hex'),
    '7f49a0a505f3ca42b3a471a023ee72f4428fe4eb563d3d957ef7428cc45e289a',
  );
});
...
test('shipped model instructions do not invite severity or urgency escalation', () => {
  const urgencyGrounding = 'Do not describe a finding as critical, severe, urgent, or as requiring immediate action unless the severity value supplied for that specific advisory supports that language.';

  for (const mode of ['api', 'offline']) {
    const instructions = buildBombotInstructions(mode);

    for (const removedText of [
      'Use clear severity tags: "HIGH severity vulnerability"',
      '🚨',
      'Clear verdict (Safe/Vulnerable/Critical)',
      'Immediate Action',
      'Multiple Critical Issues',
      'appears secure',
    ]) {
      assert.equal(instructions.includes(removedText), false, `${mode} instructions contain ${removedText}`);
    }
    assert.ok(instructions.includes(urgencyGrounding), `${mode} instructions omit urgency grounding`);
  }
});
