...
test('MAX_HISTORY_TURNS defaults to 20 when unset', async () => {
  const { config } = await loadConfig(localConfiguration());

  assert.equal(config.MAX_HISTORY_TURNS, 20);
});

test('MAX_HISTORY_TURNS requires a positive integer', async () => {
  for (const invalidValue of ['0', '-1', '3.7']) {
    await assertConfigurationRejects(
      localConfiguration({ MAX_HISTORY_TURNS: invalidValue }),
      'MAX_HISTORY_TURNS',
    );
  }
});
