...
const CONFIGURATION_VARIABLES = [
  ...
  'ENABLE_MODEL_TOGGLE',
  'DEMO_ACCESS_TOKEN',
  'HOSTED_SESSION_HOURLY_LIMIT',
  'HOSTED_IP_HOURLY_LIMIT',
  'HOSTED_PROCESS_REQUEST_BUDGET',
  'HOSTED_MAX_TURNS_PER_CONVERSATION',
...
test('valid hosted config is frozen and applies non-decoding defaults', async () => {
  ...
  assert.deepEqual(config, {
    ...
    HOSTED_SESSION_HOURLY_LIMIT: 20,
    HOSTED_IP_HOURLY_LIMIT: 60,
    HOSTED_PROCESS_REQUEST_BUDGET: 200,
    HOSTED_MAX_TURNS_PER_CONVERSATION: 20,
...
test('hosted guard limits accept explicit conservative overrides', async () => {
  const { config } = await loadConfig(localConfiguration({
    HOSTED_SESSION_HOURLY_LIMIT: '7',
    HOSTED_IP_HOURLY_LIMIT: '17',
    HOSTED_PROCESS_REQUEST_BUDGET: '0',
    HOSTED_MAX_TURNS_PER_CONVERSATION: '9',
    MAX_USER_MESSAGE_CHARACTERS: '1234',
    DEMO_ACCESS_TOKEN: 'synthetic-demo-token',
  }));

  assert.equal(config.HOSTED_SESSION_HOURLY_LIMIT, 7);
  assert.equal(config.HOSTED_IP_HOURLY_LIMIT, 17);
  assert.equal(config.HOSTED_PROCESS_REQUEST_BUDGET, 0);
  assert.equal(config.HOSTED_MAX_TURNS_PER_CONVERSATION, 9);
  assert.equal(config.MAX_USER_MESSAGE_CHARACTERS, 1234);
  assert.equal(config.DEMO_ACCESS_TOKEN, 'synthetic-demo-token');
});
